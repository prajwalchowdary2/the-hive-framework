...
class HivePipeline:
    ...
    def __init__(self, settings: dict):
        ...
        self.agents = {
            ...
            "deception": DeceptionAgent(**agent_kwargs),
            "explain": ExplainAgent(**agent_kwargs),
        }
    ...
    def _init_llm(self):
        ...
        # Auto-detect if running inside VirtualBox or Windows Sandbox VM and need to use host's Ollama
        if "localhost" in base_url or "127.0.0.1" in base_url:
            import urllib.request
            import subprocess
            import re
            
            # 1. Try default gateway from ipconfig (Windows Sandbox / Hyper-V / VirtualBox fallback)
            try:
                ...
                for gw in gateways:
                    if gw and gw != "0.0.0.0":
                        ...
                        try:
                            req = urllib.request.Request(f"{alt_url}/api/tags", method="GET")
                            with urllib.request.urlopen(req, timeout=1) as response:
                                base_url = alt_url
                                console.print(f"[green][*] Auto-detected host gateway. Redirecting LLM to host at {base_url}[/green]")
                                break
                        except Exception:
                            pass
            except Exception:
            ...
            # 2. VirtualBox standard NAT gateway fallback if ipconfig check didn't redirect
            if "localhost" in base_url or "127.0.0.1" in base_url:
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=1) as response:
                        base_url = alt_url
                        console.print(f"[green][*] Auto-detected VirtualBox guest. Redirecting LLM to host at {base_url}[/green]")
                except Exception:
                    pass
    ...
    def run(self, report_path: str, phase: str = "all") -> dict:
        ...
        pipeline_steps = [
            ...
            ("critic", "Validating plan logic (Hybrid Guardrail)", self._run_critic),
            ...
            ("deception", "Deploying active defense decoys", self._run_deception),
            ("explain", "Generating explainability narrative", self._run_explain),
        ]
    ...
    def _run_critic(self, report_path: str, results: dict) -> dict:
        return self.agents["critic"].run(results.get("planner", {}))

    def _run_recon(self, report_path: str, results: dict) -> dict:
        plan = results.get("critic", results.get("planner", {}))
    ...
    def _run_deception(self, report_path: str, results: dict) -> dict:
        plan = results.get("critic", results.get("planner", {}))
        return self.agents["deception"].run(results, plan)

    def _run_explain(self, report_path: str, results: dict) -> dict:
        plan = results.get("critic", results.get("planner", {}))
        return self.agents["explain"].run(results, plan)
