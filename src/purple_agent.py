...
class PurpleAgent(BaseAgent):
    ...
    def _generate_sigma_rules(self, techniques: list) -> list:
        ...
        for tech in techniques:
            ...
            # Validation Loop for Hallucinations & Syntax
            max_retries = 2
            valid_yaml = False
            for attempt in range(max_retries):
                ...
                try:
                    # 1. Try PySigma validation if installed
                    try:
                        from sigma.rule import SigmaRule
                        rule_obj = SigmaRule.from_yaml(rule_content)
                        valid_yaml = True
                        break
                    except ImportError:
                        pass

                    # 2. PyYAML structure validation fallback
                    parsed = yaml.safe_load(rule_content)
                    if isinstance(parsed, dict) and "title" in parsed and "logsource" in parsed and "detection" in parsed:
                        valid_yaml = True
                        break # It's valid, break the loop
                    else:
                        self.log(f"[yellow]Validation Warning:[/yellow] Generated rule for {tid} is missing required Sigma keys. Retrying...")
                except yaml.YAMLError as e:
                    self.log(f"[red]Hallucination Caught:[/red] LLM generated malformed YAML for {tid}. Error: {e}. Retrying...")
                except Exception as e:
                    self.log(f"[yellow]Rule validation error:[/yellow] {e}. Retrying...")

            if not valid_yaml:
                self.log(f"[red]Failed to generate valid Sigma rule for {tid} after {max_retries} attempts. Falling back to safe template.[/red]")
                rule_content = f"title: Fallback Detection for {tid}\nstatus: experimental\nlogsource:\n    category: process_creation\ndetection:\n    condition: all of them\n"
            ...
            rules.append({
                ...
                "validation_status": "PASSED" if valid_yaml else "FAILED_FALLBACK"
            })
